...
class DataManager(Prototypes.Singleton):
    ...
    def register_init_event(self) -> None:
        def handle_engine_init_started(e: dict) -> None:
            self.check_file_system()

        def handle_file_system_checked(e: dict) -> None:
            self.load_config(['config/config.ini'])

        def handle_server_config_sent(e: dict) -> None:
            self.scan_data_files()

        def handle_data_file_scan_finished(e: dict) -> None:
            self.load_data_files()

        listener_factory: List[tuple] = [
            (
