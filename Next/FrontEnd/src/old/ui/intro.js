...
function AnimeIntro (props) {
  React.useEffect(() => {
    const eyeMoveRate = { value: 0.0 }
    const moveEye = (e) => {
      e.preventDefault()
      const w = 30; const h = 30 // 眼动参数
      const i = document.getElementById('move_eye')
      const j = document.getElementById('move_eye_core')
      const ix = e.clientX / window.innerWidth
      const iy = e.clientY / window.innerHeight
      const tw = w * eyeMoveRate.value
      const th = h * eyeMoveRate.value
      const x = ix * tw - tw / 2
      const y = iy * th - th / 2
